...
function HomePage3() {
  return (
    <div className="bg-purpuleColor h-fit w-full flex justify-center items-center flex-col ">
      ...
      <h1 className="SpaceAGEfont mx-auto text-[42px] max-sm:text-[25px]  max-sm:text-wrap text-center text-white text-nowrap max-sm:max-w-[240]">
        design Your wAtch
      </h1>
      <div className="flex flex-row mt-10 max-lg:flex-col ">
        {numberElemnt.map((i) => (
          <div key={i.id} className="relative mx-[72] max-lg:my-28">
            <span className="absolute -right-8 -top-3 z-10">
              <Circle size={100} r={"1"} color={"white"} ops={0.5} c={40} />
            </span>
            {i.id < 4 && (
              <span className="absolute w-36 h-[1.5] top-7 left-14 bg-white max-lg:rotate-90 max-lg:top-[127] max-lg:-left-[44]"></span>
            )}
            <div className="border-1 width-14 height-14 rounded-full size-14 SpaceAGEfont text-[32px] text-center text-white">
              {i.id}
            </div>
            <p className="absolute top-20 max-lg:-top-[60]  -left-6 max-lg:-left-5 text-nowrap text-white text-[16px] tracking-widest">
              {i.text}
            </p>
          </div>
        ))}
      ...
      <div className=" w-full h-fit flex justify-between relative bg-black max-lg:flex-col max-lg:items-center ">
        ...
        <span className="relative   text-left "><Image
          ...
          height={573}
        />
        <div className="  from-black bg-gradient-to-l to-[rgb(0,0,0,.1)] w-20 h-full absolute z-20 left-[350] top-0"></div>
        <div className="  from-black bg-gradient-to-r to-[rgb(0,0,0,.03)] w-20 h-full absolute z-20 left-[0] top-0"></div>
        </span>
        <div className="flex flex-col items-start justify-center *:my-5 leading-7 max-w-[400] text-start mx-auto max-sm:items-center max-sm:my-20 text-white  ">
